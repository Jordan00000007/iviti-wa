...
function AiInference() {
    ...
    const handleAddEvent = (myData) => {
        ...
        if (myData.app_uid === params.uuid) {
            ...
            localStorage.setItem(params.uuid, JSON.stringify(tmpEvent))

            remoteVideoRef.current.setBorderOn();
            setTimeout(()=>remoteVideoRef.current.setBorderOff(), 5000 )
        }
    }
                                                ...
                                                <div className='w-100 '>
                                                    ...
                                                    <FullScreen handle={handle}>
                                                        <div className={fullScreen ? 'my-area-a2' : 'my-area-a2 position-relative'} onChange={handleVideoClick} ref={videoPanelRef} onContextMenu={handleContextMenu}>
                                                            <RemoteVideo uuid={params.uuid} status={myItem.status} onPlaying={handlePlaying} onError={handleVideoError} fullScreen={fullScreen} ref={remoteVideoRef}/>
